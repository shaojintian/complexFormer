...
# --- Configuration ---
output_path = "results/pt_llada_loss" # Changed output path
model_path = "DeepCoder-14B-Preview/" # Base for tokenizer
...
if tokenizer.mask_token is None:
    print("Tokenizer does not have a mask token. Adding '[MASK]'.")
    # Option 1: Add a new special token (recommended if vocabulary allows)
    # tokenizer.add_special_tokens({'mask_token': '[MASK]'})
    # Option 2: Use an existing unused special token (if available)
    # mask_token = '<|reserved_special_token_0|>' # Example
    # tokenizer.mask_token = mask_token
    # Option 3: Use a standard token like '<unk>' (less ideal)
    # tokenizer.mask_token = tokenizer.unk_token

    # For LLaDA, use the specific ID from the paper if possible
    # Let's assume we add '[MASK]' and resize embeddings later if needed
    added_tokens = tokenizer.add_special_tokens({'mask_token': '[MASK]'})
    print(f"Added {added_tokens} tokens. New vocab size: {len(tokenizer)}")
    mask_token_id = tokenizer.mask_token_id
    print(f"Using mask token: {tokenizer.mask_token}, ID: {mask_token_id}")
else:
    mask_token_id = tokenizer.mask_token_id
    print(f"Using existing mask token: {tokenizer.mask_token}, ID: {mask_token_id}")

# Set a specific MASK ID if required by LLaDA's pre-training, overriding tokenizer
# LLaDA paper/code uses 126336 for their 8B model. Adapt if needed.
# Check if your vocab size accommodates this ID.
# If loading from scratch, ensure this ID exists or add it.
# If fine-tuning their model, use their ID.
# For demonstration, let's assume we *want* to use 126336 if it exists,
# otherwise we use the tokenizer's default '[MASK]' ID.
llada_specific_mask_id = 126336
if llada_specific_mask_id < len(tokenizer):
     print(f"Overriding mask ID to LLaDA specific: {llada_specific_mask_id}")
     mask_token_id = llada_specific_mask_id
else:
     print(f"LLaDA specific mask ID {llada_specific_mask_id} is out of vocab bounds ({len(tokenizer)}). Using tokenizer's mask ID: {mask_token_id}")
...
try:
    ...
    # Optional: Save state even if training failed
    # trainer.save_state()
